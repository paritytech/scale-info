...
fn overflow_bug_min() {
    ...
    // TypeInfo impl roughly equivalent to that generated by #[derive(TypeInfo)]
    impl<T> TypeInfo for Foo<T>
        where
            T: TypeInfo + 'static,
            // Removing these bounds will also fix the overflow error
            T: scale::HasCompact + 'static,
    ...
    impl<T> TypeInfo for Bar<T> where T: TypeInfo + 'static {
        type Identity = Self;
