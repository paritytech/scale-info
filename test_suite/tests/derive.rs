...
use type_metadata::{
	...
	NamedField, Namespace, TypeSumClikeEnum, TypeSumEnum, TypeProductStruct, TypeProductTupleStruct, TypeDefUnion, Type,
...
fn tuple_struct_derive() {
	...
	let type_id = TypePath::new(
		...
		TypeProductTupleStruct::new(vec![UnnamedField::of::<bool>()]).into(),
	);
...
fn unit_struct_derive() {
	...
	let type_id = TypePath::new(
		...
		Namespace::new(vec!["derive"]).unwrap(),
		vec![],
		TypeProductTupleStruct::unit().into(),
	);
